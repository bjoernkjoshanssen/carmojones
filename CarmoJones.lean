...
import Myproject2.ReiffSpecialty
/-
...
def A5 [Fintype U] (ob : Finset U → Finset (Finset U)) := ∀ (X : Finset U),
  ¬ ∅ ∈ ob X

def B5 [Fintype U] [DecidableEq U] (ob : Finset U -> Finset (Finset U)) := ∀ (X Y Z : Finset U),
  (Y ∩ X = Z ∩ X) → (Y ∈ ob X ↔ Z ∈ ob X)

-- 5c_2013:
def C5 [Fintype U] [DecidableEq U] (ob : Finset U -> Finset (Finset U)) := ∀ (X Y Z : Finset U),
...
def D5 [Fintype U] [DecidableEq U] (ob : Finset U -> Finset (Finset U)) := ∀ (X Y Z : Finset U),
  Y ⊆ X → Y ∈ ob X → X ⊆ Z → (Z \ X) ∪ Y ∈ ob Z
def E5 [Fintype U] [DecidableEq U] (ob : Finset U -> Finset (Finset U)) := ∀ (X Y Z : Finset U),
  Y ⊆ X → Z ∈ ob X → Y ∩ Z ≠ ∅ → Z ∈ ob Y
def F5 [Fintype U] [DecidableEq U] (ob : Finset U -> Finset (Finset U)) := ∀ (X Y Z : Finset U),
  X ∈ ob Y → X ∈ ob Z → X ∈ ob (Y ∪ Z)
def G5 [Fintype U] [DecidableEq U] (ob : Finset U -> Finset (Finset U)) := ∀ (X Y Z : Finset U),
